...
export function UpdateBlog() {
    ...
    const [title, setTitle] = useState(state.blogData.title);
    const [summary, setSummary] = useState(state.blogData.summary);
    const [content, setContent] = useState(state.blogData.content);
    const [cover, setCover] = useState(state.blogData.cover);
    const [published, setPublished] = useState(state.blogData.published)
    const [category, setCategory] = useState(state.blogData.category)
    ...
    async function updatePost(ev: any) {
        ev.preventDefault();
        let finalCategory = newCategory ? newCategory : category

        const updateObj: any = {
            blogId: state.blogData.id
        }
        ...
        if (cover !== state.blogData.cover) {
            updateObj.cover = cover[0]
        }


        setNetworkCallInProgress(true)
        try {
            await customAxios.put(CONSTANTS.BLOG.POST_ADD_BLOG(), updateObj, { headers: { 'Content-Type': "multipart/form-data" } })
            navigate('/')
        } catch (err) {
            console.log(err)
        } finally {
        ...
        <div className="flex flex-col justify-center items-center w-full h-full">
            <div className="relative p-4 w-full max-w-2xl h-full">
                <div className="relative p-4 bg-white rounded-lg shadow gap-2 dark:bg-gray-800 sm:p-5 border border-primary-500">
                    ...
                    <form onSubmit={updatePost} className={`page-2 flex flex-col gap-4 ${currentPage === 2 ? "flex" : 'hidden'}`}>
                        <div className={`page-2 flex flex-col gap-4 `}>
                            ...
                            <div className="flex flex-row items-center justify-between">
                                ...
                                <button type="submit" className="text-white flex flex-row justify-between gap-2 items-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-semibold rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                    ...
                                    Update Blog {networkCallInProgress ? <Loader fullPage={false}></Loader> : null}
                                </button>
                            </div>
                        </div>
                    </form>
